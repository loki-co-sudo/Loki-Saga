...
作成・管理するためのリポジトリ。名前は北欧神話の「サガ(物語・年代記)」から。
...
**このリポジトリはGitHub上でpublic。** Zennの `published: false` はZenn側の表示制御であって
GitHub上の可視性とは無関係なため、`origin` へのpushは(published状態に関わらず)必ず事前に
チャットで確認してから行う。
